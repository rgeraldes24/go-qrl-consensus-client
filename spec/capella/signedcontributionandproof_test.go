...
func TestSignedContributionAndProofJSON(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name:  "JSONBad",
			input: []byte("[]"),
			err:   "invalid JSON: json: cannot unmarshal array into Go value of type capella.signedContributionAndProofJSON",
		},
		{
			name:  "MessageMissing",
		...
		},
		{
			...
			err:   "invalid JSON: invalid JSON: json: cannot unmarshal bool into Go value of type capella.contributionAndProofJSON",
		},
		{
			name:  "MessageInvalid",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			var res capella.SignedContributionAndProof
