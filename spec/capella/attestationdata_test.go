...
func TestAttestationDataJSON(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name:  "JSONBad",
			input: []byte("[]"),
			err:   "invalid JSON: json: cannot unmarshal array into Go value of type capella.attestationDataJSON",
		},
		{
			name:  "SlotMissing",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			var res capella.AttestationData
